...
export async function runChecks(editor, invoke) {
  ...
  say('S1.3: the wheel zooms around the pointer, with Ctrl or without, a sideways wheel pans, fullscreen in and out, the file name in the title');
  {
    ...
    stage.style.bottom = `${window.innerHeight - 49}px`;
  ...
  say('S2.1: the timeline along the bottom lists every document as a small thumbnail, the current one marked; a click shows that document with its notes; fullscreen puts it away');
  {
    ...
    check('the stage ends above the picture\'s size and the strip, below the top bar', stage.clientHeight === window.innerHeight - 48 - 48 - 96, `stage ${stage.clientHeight} of ${window.innerHeight}`);
    ...
    check('the picture\'s size too, with the stage ending above it', getComputedStyle(sizeEl).display === 'block' && stage.clientHeight === window.innerHeight - 48 - 48 - 96,
  ...
  say('S2.3 and S2.4: Copy and Save As are buttons beside the mode as well as keys, and the HUD says what the store holds');
  {
    ...
    check('the controls are a sidebar down the left edge, below the top bar, and the stage starts at its right edge', !controls.hidden && copyButton && saveButton && document.getElementById('mode')
      && cbox.left === 0 && Math.round(cbox.top) === 48 && Math.round(cbox.width) === 64 && stageLeft === 64,
  ...
  say('the top bar: Recon draws its own, the title on the left, minimize, maximize and close on the right; fullscreen puts it away');
  {
    ...
    check('the bar runs across the top, 48 px tall, and the stage starts below it', barBox.top === 0 && barBox.height === 48 && Math.round(barBox.width) === window.innerWidth && stageTop() === 48,
    ...
    const boxes = buttons.map((b) => b.getBoundingClientRect());
    // 48 by 48, which is the bar's whole height (Rotem, 2026-09-17).
    check('minimize, maximize and close sit in that order at the right edge, 48 by 48', buttons.map((b) => b.id).join() === 'win-min,win-max,win-close'
      && boxes.every((b) => Math.round(b.width) === 48 && Math.round(b.height) === 48 && Math.round(b.height) === bar.clientHeight && b.top === 0) && Math.round(boxes[2].right) === window.innerWidth && boxes[0].right <= boxes[1].left && boxes[1].right <= boxes[2].left,
    ...
    check('a maximized window still has the bar at the top and the stage below it', stageTop() === 48 && Math.round(document.getElementById('winbtns').getBoundingClientRect().right) === window.innerWidth);
    document.getElementById('win-max').click();
    for (let i = 0; i < 40 && (await win.isMaximized()); i += 1) await sleep(50);
    await sleep(200);
    ...
    await editor.setFullscreen(false);
    check('and they come back', getComputedStyle(bar).display !== 'none' && getComputedStyle(sidebar).display !== 'none' && stageTop() === 48 && stageLeft() === 64, `the stage from ${stageLeft()},${stageTop()}`);
  }
  ...
  say('S2.8: the strip holds a screenful of cells whatever the library holds; its top edge drags it taller, the thumbnails growing to 320 wide and then wrapping into rows of 320; the height is remembered');
  {
    ...
    check('dragged past one row of 320: the strip is exactly as dragged, 300 tall, one row of 320 by 200 cells', h1 === 300 && layout.rows === 1 && layout.w === 320 && layout.h === 200 && stage.clientHeight === window.innerHeight - 48 - 48 - 300,
    ...
    check('at 436 the second row enters: two rows of 320, 12 px apart, as many columns as fit, scrolled vertically', h2 === 436 && layout.rows === 2 && layout.cols === cols && strip.classList.contains('grid') && second.x === 12 && second.y === 224
      && strip.scrollHeight === 24 + Math.ceil(30 / cols) * 212 - 12 && stage.clientHeight === window.innerHeight - 48 - 48 - 436,
    ...
    check('and between rows the height is the hand\'s, the rows unchanged', h2b === 500 && editor.stripLayout().rows === 2 && stage.clientHeight === window.innerHeight - 48 - 48 - 500, `${h2b} tall, ${editor.stripLayout().rows} rows`);
    ...
    check('at the ceiling the picture\'s size stops 16 px under the top bar, and the strip covers it', Math.round(cappedSize.top) === 64 && strip.contains(cappedHit),
    ...
    try { kept = localStorage.getItem('recon.strip-height'); } catch (_) { /* none */ }
    check('a double-click on the edge returns the strip to its default, 137 tall: the line, 12 px, 112 px thumbnails ending at 124, the band of the scroller under them', editor.stripHeightOf() === 137 && kept === '137' && editor.stripLayout().h === 112 && stage.clientHeight === window.innerHeight - 48 - 48 - 137 && strip.clientHeight === 124 && strip.children[0].offsetTop + strip.children[0].offsetHeight === 124,

...
export async function runChecks(editor, invoke) {
  ...
  say('Settings: a chevron left of minimize opens a modal; a shortcut for opening Recon and two for the capture are picked by pressing them; a taken one is refused in words and the old one stays; the file keeps all three');
  {
    ...
    // The switch for the mouse pointer in a capture (Rotem, 2026-09-21).
    const pointerRow = panel.querySelector('.setting[data-switch="pointer"]');
    const pointerSwitch = pointerRow.querySelector('.switch');
    const switchBox = pointerSwitch.getBoundingClientRect();
    check('a row under Capture 2, Mouse pointer in captures, holds a switch that is on, with nothing in the file about it', pointerRow.querySelector('span').textContent === 'Mouse pointer in captures'
      && Math.round(pointerRow.getBoundingClientRect().top) > Math.round(row('capture2').getBoundingClientRect().top) && pointerSwitch.getAttribute('role') === 'switch'
      && pointerSwitch.getAttribute('aria-checked') === 'true' && Math.round(switchBox.width) === 32 && Math.round(switchBox.height) === 18 && !('capture_pointer' in (await file())),
      `${pointerSwitch.getAttribute('aria-checked')} ${switchBox.width}x${switchBox.height} ${JSON.stringify(await file())}`);
    pointerSwitch.click();
    await settled(() => pointerSwitch.getAttribute('aria-checked') === 'false');
    check('a click switches it off, in the file too, and the shortcuts stay', pointerSwitch.getAttribute('aria-checked') === 'false' && (await file()).capture_pointer === false && (await file()).hotkey === 'Ctrl+Shift+5',
      JSON.stringify(await file()));
    pointerSwitch.click();
  ...
  // ---------------------------------------------------------------- 46. the mouse pointer a capture caught
  say('');
  say('The mouse pointer (Rotem, 2026-09-21): a capture that caught the pointer arrives with it as an element on top, at its place and its own size, never in the capture\'s pixels; it is in the copy and the thumbnail; a drag moves it, the x on hover deletes it, Ctrl+Z and Ctrl+Shift+Z walk both, Ctrl+Z never takes the arrival off; it is back after a restart and never arrives twice');
  {
    const stage = editor.stage;
    await invoke('editor_store_reset');
    const shot = await invoke('editor_capture_probe_pointer', { width: 400, height: 300, x: 120, y: 90 });
    const arrived = async () => { for (let i = 0; i < 60 && !(model.image.document_id === shot.document_id && model.shapes.some((sh) => sh.kind === 'pointer')); i += 1) await sleep(50); };
    await arrived();
    await sleep(300); // the capture's own copy, which follows the arrival
    await editor.setZoom(1);
    const box = stage.getBoundingClientRect();
    const css = (ix, iy) => ({ x: box.left + model.offset.x + ((ix - model.pan.x) * model.zoom) / editor.ratioOf(), y: box.top + model.offset.y + ((iy - model.pan.y) * model.zoom) / editor.ratioOf() });
    const pointer = (type, target, ix, iy) => {
    ...
    const press = (init) => { const e = new KeyboardEvent('keydown', { bubbles: true, cancelable: true, ...init }); window.dispatchEvent(e); return e.defaultPrevented; };
    const pointerEl = () => document.querySelector('#scene .pointer[data-shape]');
    const place = () => { const sh = model.shapes.find((x) => x.kind === 'pointer'); const el = pointerEl(); return `${sh ? `${sh.a.x},${sh.a.y} to ${sh.b.x},${sh.b.y}` : 'no shape'}; box ${el ? `${el.style.left} ${el.style.top} ${el.style.width} ${el.style.height}` : 'none'}`; };
    // A pixel of the exported layer, which is what the host lays over the capture.
    const layerPixel = async (layer, x, y) => {
      const bitmap = await createImageBitmap(new Blob([layer.bytes], { type: 'image/png' }));
      const canvas = document.createElement('canvas');
    ...
    check('the capture said a pointer waited, 32 by 32 at 120,90', !!shot.pointer && shot.pointer.x === 120 && shot.pointer.y === 90 && shot.pointer.width === 32 && shot.pointer.height === 32, JSON.stringify(shot.pointer));
    const shape = model.shapes.find((sh) => sh.kind === 'pointer');
    check('it arrived as one element on top, a box in the scene at its place and its own size, its picture a PNG of its own', model.shapes.length === 1 && !!shape && !!pointerEl() && pointerEl().style.left === '120px' && pointerEl().style.top === '90px'
      && pointerEl().style.width === '32px' && pointerEl().style.height === '32px' && String(shape.src).startsWith('data:image/png;base64,') && pointerEl().querySelector('img').naturalWidth === 32, place());
    check('the capture under it is whole: the picture\'s own pixel there is not the pointer\'s red', (await sourcePixel(124, 94)) !== '255,0,0', await sourcePixel(124, 94));
    check('the arrival is no step of history, so Ctrl+Z has nothing to take off', model.history.steps.length === 1 && model.history.index === 0 && model.history.steps[0].includes('"pointer"'), `${model.history.steps.length} steps, ${model.shapes.length} shapes`);
    const again = await (await fetch(`http://region.localhost/?pointer=${shot.document_id}`, { cache: 'no-store' })).status;
    check('the host hands the picture over once: a second ask finds nothing, and the capture no longer says a pointer waits', again === 404 && (await invoke('editor_image_info')).pointer === null, `status ${again}`);

    // The copy: the pointer's red where it is solid, nothing where it is clear, and no x.
    let layer = await editor.exportLayer();
    const solid = await layerPixel(layer, model.margin.left + 124, model.margin.top + 94);
    const clear = await layerPixel(layer, model.margin.left + 144, model.margin.top + 114);
    check('the export carries the pointer: red where its picture is solid, nothing where it is clear, and not the x', solid === '255,0,0,255' && clear === '0,0,0,0' && !layer.markup.includes('<button') && !layer.markup.includes('data-ui'), `solid ${solid}, clear ${clear}`);

    // The x, the ruler's own.
    const xEl = () => pointerEl() && pointerEl().querySelector('.x');
    const xs = getComputedStyle(xEl());
    check('its x is the ruler\'s: a round 16 by 16 button, hidden until hover', xs.display === 'none' && xs.width === '16px' && xs.height === '16px' && xs.borderRadius === '8px' && xEl().getAttribute('aria-label') === 'Delete', `${xs.display} ${xs.width}x${xs.height}`);

    // Moved whole, one step; undone and redone.
    pointer('pointerdown', pointerEl(), 130, 100);
    pointer('pointermove', stage, 180, 140);
    pointer('pointerup', stage, 180, 140);
    check('a drag moves it whole, one step of history', shape.a.x === 170 && shape.a.y === 130 && shape.b.x === 202 && shape.b.y === 162 && pointerEl().style.left === '170px' && pointerEl().style.top === '130px' && model.history.index === 1, place());
    layer = await editor.exportLayer();
    const moved = await layerPixel(layer, model.margin.left + 174, model.margin.top + 134);
    const left = await layerPixel(layer, model.margin.left + 124, model.margin.top + 94);
    check('the export follows the move: red at the new place, nothing at the old', moved === '255,0,0,255' && left === '0,0,0,0', `new ${moved}, old ${left}`);
    press({ key: 'z', code: 'KeyZ', ctrlKey: true });
    check('Ctrl+Z puts it back where it arrived', !!pointerEl() && pointerEl().style.left === '120px' && pointerEl().style.top === '90px', place());
    press({ key: 'Z', code: 'KeyZ', ctrlKey: true, shiftKey: true });
    check('Ctrl+Shift+Z moves it again', !!pointerEl() && pointerEl().style.left === '170px' && pointerEl().style.top === '130px', place());
    press({ key: 'Escape', code: 'Escape' });

    // Saved with the notes, in the thumbnail, back after a restart, and not a second time.
    await editor.saveNow();
    await editor.thumbnailDone();
    editor.documents.clear();
    model.shapes = [];
    ...
    editor.layoutScene();
    await editor.loadImage(await invoke('editor_store_reload'));
    const back = model.shapes.find((sh) => sh.kind === 'pointer');
    check('after a restart it is back from the disk, picture and place, once', model.shapes.length === 1 && !!back && back.a.x === 170 && back.a.y === 130 && String(back.src).startsWith('data:image/png;base64,') && document.querySelectorAll('#scene .pointer').length === 1, place());

    // The x deletes it; undo and redo walk the deletion.
    await editor.setMode('annotate');
    pointer('pointerdown', xEl(), 202, 130);
    pointer('pointerup', stage, 202, 130);
    check('a press on the x deletes it', model.shapes.length === 0 && !pointerEl());
    layer = await editor.exportLayer();
    const gone = await layerPixel(layer, model.margin.left + 174, model.margin.top + 134);
    check('and the export no longer carries it', gone === '0,0,0,0', gone);
    press({ key: 'z', code: 'KeyZ', ctrlKey: true });
    check('Ctrl+Z brings it back, picture and all', model.shapes.length === 1 && !!pointerEl() && pointerEl().querySelector('img').src.startsWith('data:image/png;base64,') && pointerEl().style.left === '170px', place());
    press({ key: 'Z', code: 'KeyZ', ctrlKey: true, shiftKey: true });
    check('Ctrl+Shift+Z deletes it again', model.shapes.length === 0 && !pointerEl());

    // A capture with no pointer arrives bare.
    const bare = await invoke('editor_capture_probe', { width: 400, height: 300 });
    for (let i = 0; i < 60 && model.image.document_id !== bare.document_id; i += 1) await sleep(50);
    await sleep(300);
    check('a capture that caught no pointer arrives with none', bare.pointer === null && model.shapes.length === 0 && !pointerEl(), JSON.stringify(bare.pointer));

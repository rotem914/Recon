...
| The capture overlay | Build with `--features stage0-checks` and run `--selftest`: C drags, D escapes, H clicks a window near its corner for the whole window, clicks its one child for the part, drags from the same point, all through the real overlay, and writes the lit part beside the executable to be looked at. C also copies the screen beside the pointer mid-drag to `s02-drag-size.png` beside the executable and reads it: the size bubble's ground in the app background 12 px right of and 19 px below the pointer, and the light pixels of its text; the numbers in the bubble, its corners and its distance from the pointer are looked at in the file. A real hotkey drag by hand is Rotem's hand. |

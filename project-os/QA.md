...
| The scrolling capture | Build with `--features stage0-checks` and run `--selftest`, section I, or `--selftest --only-scroll` for it alone: a stand-in window scrolls a 2400 row page made from a formula; the round button's #2554FB is looked for on the screen at the lit window's bottom middle, 24 px up, and its lighter blue under the pointer; a click on it hands back the lit frame whole, the stand-in's visible bounds read from the window manager, never the smaller part of it that scrolls (Rotem's bug of 2026-09-22); the bar's window is visible, 24 px up from the frame's bottom and in its middle, and a copy of the screen where it lies is the page alone; Escape ends it with nothing; then the wheel scrolls the page to its end and Enter ends it, and the tall picture is 2400 rows and the frame's own taller, the title bar once at its top, every pixel of the page equal to the formula's but the two bottom corners Windows rounds. It would turn red on a row joined twice or left out, on the overlay or the bar in the picture, on a scrollbar cut where there is none. The bar's and the button's looks are written to `s02-scrolling-looks.png` to be looked at, since Windows keeps the bar out of every screen copy. `cargo test -p recon-pixels stitch`: steps of every size, a jump too far lost and found again, a scroll back up, bars stuck top and bottom with a side menu and a scrollbar, round corners, the longest picture, a frame that did not move but whose edge columns changed read as unchanged, a scroll beside a still part three fifths wide read as the scroll, a jump too far beside that still part lost, and rows repeating at a pitch never making a still frame a move. `cargo test --all-features scrolling::detect`: sections that follow each other past a panel are one run, the design system page scrolls at 100 to 400%, a skip link parked above a page that fits is no scroll, a child far off on its own is left out. `--selftest --scroll-trial <title>` tries it on a real window and writes the picture to be looked at. It takes the real mouse and wheel, and another check running beside it breaks both. A real browser page scrolled by hand, fast and slow, and the picture in the editor are Rotem's hand. |
...
| A capture with Ctrl held | `--editor-check`, its last section: with the window hidden, a probe announced as a Ctrl capture is loaded by the page and copied, the clipboard's PNG equal to that probe's composition at its 330 by 210, the window still hidden, the capture in the timeline; with the clipboard held, the window comes up on that capture saying NOT COPIED, and hidden again it returns the focus to the stand-in the capture began in, where nothing would be returned to without the set-aside target; a probe without Ctrl brings it up as before. `--selftest`, section J: through the real overlay, a drag released without Ctrl is recorded without it, a drag and a click on the stand-in's part released with Ctrl down are recorded with it, and the record is gone once taken. A real Ctrl capture by the shortcut, with the editor hidden, minimized and open behind, then a paste, is Rotem's hand. |
...
| The timeline's ground | `--editor-check`, S2.8: the strip's background equal to the page's own, a 0 px top line, the first thumbnail 16 px under the tabs' bottom edge; the default 124 tall with 112 px thumbnails ending at 112, the second row entering at 424. S2.1: the band between the stage and the strip 64 tall, the size 24 px and the tabs 16 px from both of its edges. A taller strip still covers the sidebar's buttons and the size. A look at the real timeline is Rotem's hand. |
...
| The mouse pointer in a capture | `cargo test`: a 2 by 2 pointer drawn into a 3 by 3 frame left of the primary display lands pixel by pixel where it stood, blended by its alpha, the frame solid throughout, cut at the corner, and nothing drawn when it is outside; the alpha from two drawings; a box landing in a frame; the switch kept across a shortcut save. `cargo test --all-features pointer -- --ignored`, on a desktop with a pointer showing: the real pointer read and written to `pointer-grab.png`, and the real screen frozen with the pointer drawn in, no pixel outside its box changed, the 160 px around it written to `pointer-in-frame.png`, both beside the test executable, to be looked at. `--selftest`: after the drag the spot recorded for the pointer is the corner the mouse was released on, and after the click on the stand-in the spot it clicked, each against the coordinates the test sent. `--editor-check`, Settings: the switch on with no key in the file, off and on again written as `capture_pointer`. A real capture with the pointer in it, with the switch off, over a text field and on a scaled display, is Rotem's hand. |

...
| The mouse pointer in a capture | `--editor-check` section 46: a probe capture with a 32 by 32 pointer, a red square in a clear box, arrives with one element at its place and size, the capture's own pixel under it not red, the history one step; a second ask of the host for the picture is a 404; the exported layer is red where the pointer is solid and clear beside it, with no x; a drag moves it, one step, the export following; Ctrl+Z and Ctrl+Shift+Z walk the move and the x's delete; it is back from the disk after a restart, once; a probe without one arrives bare. Settings: the switch on with no key in the file, off and on again written as `capture_pointer`. `cargo test`: the alpha from two drawings, a box landing in a selection, the switch kept across a shortcut save. `cargo test --all-features pointer -- --ignored` reads the real pointer and writes `pointer-grab.png` beside the test executable, to be looked at. A real capture with the pointer in it, over a text field and on a scaled display, is Rotem's hand. |

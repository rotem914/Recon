...
| The timeline's tabs | `--editor-check` section 40: the plus 32 by 32 at the band's left with a 20 by 20 icon, the first press making Main and New tab, a probe capture on a tab landing in its feed and in Main, the list read back from `tabs.json` as saved, a pointer drag putting the last tab right after Main and Main refusing a drag, the × deleting the selected tab and Main refusing, fullscreen away and back with the timeline. A real capture on a tab, and a drag by hand, are Rotem's hand. |

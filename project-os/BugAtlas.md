...
| 4 | A delete, or any move of a document's folder, is refused with "Access is denied" now and then, most often right after the timeline refreshed | Windows refuses to move a folder while a file in it is open, and a thumbnail being read or made on its thread holds one for a few milliseconds; before review 3 the refusal was logged and the document was dropped from the list anyway, to come back at the next startup | Retry the rename for up to a second on a permission error (`store::trash`), and never take the document off the list before the move succeeded; a refusal that outlasts the second is returned to the page, which says NOT DELETED | 1 | History 2026-09-16, "Review 3 fixed, T1 to T11"; the review document, T2 and T10 |
